...
export default async function HandymanMessagesPage() {
  ...
  if (!user) {
    redirect("/auth/login?redirectTo=/handyman/messages");
  }
  ...
  // Get bookings assigned to this handyman (via assigned_tech_user_id which is the profile_id)
  const { data: bookings } = await supabase
    .from("bookings")
    .select("id")
    .eq("assigned_tech_user_id", user.id) as { data: { id: string }[] | null };
