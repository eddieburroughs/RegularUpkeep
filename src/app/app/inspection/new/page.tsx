...
export default function NewInspectionPage() {
  ...
  const loadProperties = useCallback(async () => {
    ...
    const { data } = await supabase
      ...
      .select("properties(id, nickname, address_line1, city, state)")
      .eq("user_id", user.id)
      .eq("is_active", true) as { data: { properties: Property }[] | null };
  ...
  const _uploadPhotos = async (docId: string): Promise<string[]> => {
    ...
    for (const file of photoFiles) {
      ...
      const path = `inspections/${docId}/${currentSection?.key}/${currentItem?.key}/${crypto.randomUUID()}.${ext}`;
  ...
  const goToNext = async () => {
    // Upload photos for current item if any
    if (photoFiles.length > 0 && currentSection && currentItem) {
      // Photos will be uploaded during final save
      const currentPhotos = getCurrentFinding().photos;
      // Store file references temporarily (we'll upload during save)
      ...
      for (const [, sectionFindings] of Object.entries(findings)) {
        for (const [, finding] of Object.entries(sectionFindings)) {
          ...
          if (pendingPhotos.length > 0) {
            ...
            // In real implementation, store files in state keyed by section/item
          }
        }
      }
            ...
            if (finding.createTask && finding.status === "attention") {
              ...
              const { data: task } = await (supabase.from("maintenance_tasks") as any)
                .insert({
                  ...
                  description: finding.notes || `From inspection on ${today}`,
        ...
        for (const [sectionKey, sectionFindings] of Object.entries(findings)) {
          for (const [itemKey, finding] of Object.entries(sectionFindings)) {
            if (finding.createRequest && finding.status === "urgent") {
              ...
              // Create a maintenance task marked as urgent (could also use a requests table)
              // eslint-disable-next-line @typescript-eslint/no-explicit-any
              const { data: task } = await (supabase.from("maintenance_tasks") as any)
                .insert({
                  property_id: selectedPropertyId,
                  name: `URGENT: ${item?.label || itemKey}`,
                  description: finding.notes || `Urgent issue found during inspection on ${today}. Professional service needed.`,
                  category: sectionKey === "plumbing" ? "plumbing"
                    : sectionKey === "hvac" ? "hvac"
                    : sectionKey === "electrical" ? "electrical"
                    : sectionKey === "exterior" ? "exterior"
      ...
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-muted-foreground">Set up your home walkthrough</p>
          </div>
        </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Choose the depth of your inspection</CardDescription>
          </CardHeader>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <label htmlFor="saveReport">Save inspection report to Binder</label>
            </div>
            {attentionItems.length > 0 && (
              <div className="flex items-center gap-3">
            ...
            {urgentItems.length > 0 && (
              <div className="flex items-center gap-3">
                ...
                <label htmlFor="createRequests">Create service requests for urgent items ({urgentItems.filter(i => i.createRequest).length})</label>
              </div>
            )}
          </CardContent>
        </Card>
      ...
      <div className="space-y-6">
        <div className="text-center space-y-4 py-8">
          ...
          <p className="text-muted-foreground">
            Your inspection report has been saved to your Binder.
          </p>
        </div>
        ...
        <div className="space-y-3">
          {createdDocId && (
            <Button asChild className="w-full" size="lg">
              <Link href={`/app/inspection/${createdDocId}`}>
                View Report
                <ArrowRight className="ml-2 h-4 w-4" />
              </Link>
            </Button>
          ...
          {createTasks && summary.attention > 0 && (
            <Button variant="ghost" asChild className="w-full">
              <Link href="/app/calendar">View Created Tasks</Link>
            </Button>
          )}
        </div>
      </div>
