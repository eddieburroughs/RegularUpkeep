...
export default function ProviderNewInspectionPage() {
  ...
  // Load properties from provider's bookings
    ...
    try {
      ...
      const { data: doc, error: docError } = await (supabase.from("documents") as any)
        .insert({
          ...
          category: "inspection",
          title: `Provider Inspection – ${propertyName} – ${today}`,
            ...
            if (finding.createTask && finding.status === "attention") {
              ...
              const { data: task } = await (supabase.from("maintenance_tasks") as any)
                .insert({
                  ...
                  description: finding.notes || `From provider inspection on ${today}`,
            ...
            if (finding.createRequest && finding.status === "urgent") {
              ...
              const { data: task } = await (supabase.from("maintenance_tasks") as any)
                .insert({
                  ...
                  description: finding.notes || `Urgent issue found during provider inspection on ${today}`,
                  category: sectionKey === "plumbing" ? "plumbing"
                    : sectionKey === "hvac" ? "hvac"
                    : sectionKey === "electrical" ? "electrical"
                    : sectionKey === "exterior" ? "exterior"
        ...
        <div className="space-y-3">
          {createdDocId && (
            <Button asChild className="w-full" size="lg">
              <Link href={`/provider/inspection/${createdDocId}`}>
                View Report
                <ArrowRight className="ml-2 h-4 w-4" />
              </Link>
            </Button>
